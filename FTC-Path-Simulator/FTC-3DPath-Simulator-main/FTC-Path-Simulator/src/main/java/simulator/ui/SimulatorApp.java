...
import simulator.io.AIAnalyzer;
...
public class SimulatorApp {
    ...
    // camera orbit — full 360° spherical with pan
    private double camAzim = 45, camElev = 40, camDist = 140;
    private double camPanX = 0, camPanZ = 0;       // right-drag pan offset
    private double lastMX, lastMY;
    private boolean rightDrag;                      // which button is active
    private Group camPivot, camTilt, camPan;
    ...
    private final Button viewBtn    = new Button("🎥 Reset View");
    private final TextField aiKeyField = new TextField();
    private final ComboBox<String> speedCombo = new ComboBox<>();
    private final AIAnalyzer ai = new AIAnalyzer();

    // ---- simulation ----
    private AnimationTimer animTimer;
    private long lastFrameNanos;
    private boolean running;
    private double simSpeed = 3.0;   // simulation speed multiplier
    ...
    public SimulatorApp() {
        ...
        // Place ball at the centre of the field.
        this.engine.getBallEngine().placeAt(0, 0, Ball.DEFAULT_RADIUS);
        buildUI();
        build3DScene();
        updateCamera();
    }
    ...
    private VBox buildLeftPanel() {
        ...
        // Robot size
        Label dimTitle = boldLabel("Robot Size (inches)");
        ...
        motGrid.add(new Label("Motor:"), 0, 0);   motGrid.add(chassisMotorCombo, 1, 0);

        // Program
        Label progTitle = boldLabel("Auto Program  (paste Java or commands)");
        programArea.setPrefRowCount(20);
        programArea.setFont(Font.font("Monospaced", 11));
        programArea.setText("MOVE_TO(24, 24)\nINTAKE\nMOVE_TO(-48, -12)\nLAUNCH\n");
        VBox.setVgrow(programArea, javafx.scene.layout.Priority.ALWAYS);
        ...
        viewBtn.setPrefWidth(260);
        viewBtn.setOnAction(e -> resetCamera());

        // Speed control
        speedCombo.getItems().addAll("1x", "2x", "3x", "5x", "10x", "20x");
        speedCombo.setValue("3x");
        speedCombo.setOnAction(e -> {
            String v = speedCombo.getValue();
            simSpeed = Double.parseDouble(v.replace("x", ""));
        });
        Label speedLabel = new Label("Sim Speed:");
        speedLabel.setFont(Font.font("SansSerif", FontWeight.BOLD, 12));
        HBox speedBox = new HBox(8, speedLabel, speedCombo);
        speedBox.setAlignment(javafx.geometry.Pos.CENTER_LEFT);

        // AI API key
        Label aiLabel = new Label("AI Key (optional):");
        aiLabel.setFont(Font.font("SansSerif", FontWeight.BOLD, 12));
        aiKeyField.setPromptText("sk-... or leave empty for regex parser");
        HBox aiBox = new HBox(8, aiLabel, aiKeyField);
        aiBox.setAlignment(javafx.geometry.Pos.CENTER_LEFT);

        panel.getChildren().addAll(
            dimTitle, dimGrid, new Separator(),
            motGrid, new Separator(),
            progTitle, programArea, new Separator(),
            startBtn, resetBtn, uploadBtn, viewBtn, speedBox, aiBox);
        return panel;
    }
    ...
    private void uploadOpMode() {
        ...
        try {
            ...
            System.out.println("[UPLOAD] Parsed " + cmds.size() + " commands from " + f.getName());
            if (cmds.isEmpty()) {
    ...
    private void build3DScene() {
        ...
        // --- camera with full 360° orbit + pan hierarchy ---
        //  camPivot (Y=azimuth) → camTilt (X=elevation) → camPan (XY translate) → camera (Z=distance)
        camera = new PerspectiveCamera(true);
        camera.setFieldOfView(45);
        camera.setNearClip(1);
        camera.setFarClip(800);
        camera.setTranslateZ(-camDist);
        camPan  = new Group(camera);
        camTilt = new Group(camPan);
        ...
        // ---- mouse controls: 360° orbit + pan + zoom ----
        subScene.setOnMousePressed(e -> {
            lastMX = e.getSceneX();
            lastMY = e.getSceneY();
            rightDrag = e.isSecondaryButtonDown();
        });
        subScene.setOnMouseDragged(e -> {
            ...
            if (e.isSecondaryButtonDown()) {
                // Right-drag: pan the view in screen space.
                // Scale factor depends on distance for consistent feel.
                double panScale = camDist / 300.0;
                camPanX += dx * panScale;
                camPanZ -= dy * panScale;
            } else {
                // Left-drag: orbit — full 360° spherical rotation.
                camAzim -= dx * 0.3;
                camElev += dy * 0.3;
                // No clamping — full vertical orbit; avoid gimbal lock at ±90°.
                if (camElev > 89.5)  camElev = 89.5;
                if (camElev < -89.5) camElev = -89.5;
            }
            updateCamera();
        });
        subScene.setOnScroll(e -> {
            ...
            camDist = Math.max(30, Math.min(400, camDist));
            updateCamera();
        });
    }
    ...
    private void updateCamera() {
        camera.setTranslateZ(-camDist);
        camPan.getTransforms().setAll(
            new Translate(camPanX, 0, camPanZ));
    ...
    /** Reset the camera to the default overhead view. */
    ...
    private void updateBall() {
        ...
        // Always show the ball at its current position, even when idle.

        // Ball sphere — size matches the Ball's actual radius.
        Sphere ballSphere = new Sphere(b.getRadius());
        // Color shifts with speed: hotter = faster.
        double speed = b.getSpeed();
        double heat = Math.min(1.0, speed / 600.0);   // normalise up to ~600 in/s
        PhongMaterial dynMat = mat(Color.rgb(
            (int)(255 * (0.85 + 0.15 * heat)),
            (int)(215 * (1.0 - 0.7 * heat)),
            (int)(50  * (1.0 - 0.8 * heat)),
            0.95));
        ballSphere.setMaterial(dynMat);
        ballSphere.setTranslateX(b.getX());
        ballSphere.setTranslateZ(b.getY());
        ballSphere.setTranslateY(b.getZ());    // world height → +Y (up)
        ballGroup.getChildren().add(ballSphere);

        // Elasticity indicator ring — small sphere at current position.
        Sphere elasticDot = new Sphere(b.getRadius() * 0.35);
        elasticDot.setMaterial(matBallTr);
        elasticDot.setTranslateX(b.getX());
        elasticDot.setTranslateZ(b.getY());
        elasticDot.setTranslateY(b.getZ() + b.getRadius() + 0.3);
        ballGroup.getChildren().add(elasticDot);

        // Trajectory trail with varying opacity.
        List<double[]> traj = be.getTrajectory();
        int total = traj.size();
        for (int i = 0; i < total; i += 2) {   // skip every other for perf
            double[] pt = traj.get(i);
            double alpha = 0.25 + 0.45 * ((double) i / Math.max(1, total));
            Sphere s = new Sphere(b.getRadius() * 0.35);
            PhongMaterial trailMat = mat(Color.rgb(
                255, 200, 50, alpha));
            s.setMaterial(trailMat);
            s.setTranslateX(pt[0]);
            s.setTranslateZ(pt[1]);
            s.setTranslateY(pt[2]);
    ...
    private void startSimulation() {
        ...
        try {
            ...
            profile = new RobotProfile(
                ...
                2.0,
                MotorType.GOBILDA_5203_19_2,   // launcher: default
                45.0, 1.5,
                false,                          // vision: off
                RobotProfile.IntakeType.INTAKE_ROLLER,
                RobotProfile.ScoringCapability.CLASSIFIER_ONLY);
        } catch (Exception ex) {
        ...
        engine.getBallEngine().placeAt(0, 0, Ball.DEFAULT_RADIUS);

        // Auto-detect Java source and parse it before loading.
        String raw = programArea.getText();
        if (raw != null && (raw.contains("package ") || raw.contains("import ")
                || raw.contains("public class ") || raw.contains("OpMode")
                || raw.contains("runOpMode"))) {

            List<String> parsed = null;

            // Try AI first if configured.
            ai.setApiKey(aiKeyField.getText().trim());
            if (ai.isReady()) {
                statusLabel.setText("[AI] Analyzing Java code...");
                parsed = ai.analyze(raw);
                if (!parsed.isEmpty()) {
                    System.out.println("[SIM] AI parsed → " + parsed.size() + " commands");
                }
            }

            // Fall back to regex parser.
            if (parsed == null || parsed.isEmpty()) {
                parsed = new JavaCodeParser().parseContent(raw);
                if (!parsed.isEmpty()) {
                    System.out.println("[SIM] Regex parsed → " + parsed.size() + " commands");
                }
            }

            if (!parsed.isEmpty()) {
                StringBuilder sb = new StringBuilder();
                for (String c : parsed) sb.append(c).append('\n');
                programArea.setText(sb.toString().trim());
            }
        }
        ...
        System.out.println("[SIM] Final program:\n" + programArea.getText());
